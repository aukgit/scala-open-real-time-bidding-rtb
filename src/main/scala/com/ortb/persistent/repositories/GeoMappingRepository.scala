...
class GeoMappingRepository(appManager : AppManager)
  extends
    RepositoryBase[Geomapping, GeomappingRow, Int](appManager) {
  ...
  override def setEntityId(
    entityId : Option[Int],
    entity   : Option[Tables.GeomappingRow]) : Option[Tables.GeomappingRow] = {
  ...
  override def getAddAction(
    entity : Tables.GeomappingRow) =
    table returning table.map(_.geomappingid) into
      ((entityProjection, entityId) => entityProjection.copy(geomappingid = entityId)) += entity
  ...
  override def getQueryById(
    id : Int) = table.filter(c => c.geomappingid === id)
