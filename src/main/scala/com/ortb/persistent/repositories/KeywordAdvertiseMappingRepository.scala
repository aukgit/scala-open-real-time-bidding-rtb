...
class KeywordAdvertiseMappingRepository(appManager : AppManager)
  extends
    RepositoryBase[Keywordadvertisemapping, KeywordadvertisemappingRow, Int](appManager) {

  override def tableName : String = this.keywordAdvertiseMappingTableName

  override def getEntityId(entity : Option[Tables.KeywordadvertisemappingRow]) : Int =
    if (entity.isDefined) entity.get.keywordadvertisemappingid
    else -1

  override def setEntityId(
    entityId : Option[Int],
    entity   : Option[Tables.KeywordadvertisemappingRow]) : Option[Tables.KeywordadvertisemappingRow] = {
  ...
  override def getAddAction(
    entity : Tables.KeywordadvertisemappingRow) =
    table returning table.map(_.keywordadvertisemappingid) into
      ((entityProjection, entityId) => entityProjection.copy(keywordadvertisemappingid = entityId)) += entity
  ...
  override def getQueryById(
    id : Int) = table.filter(c => c.keywordadvertisemappingid === id)
