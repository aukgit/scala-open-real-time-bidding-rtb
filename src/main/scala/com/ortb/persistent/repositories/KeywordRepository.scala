...
class KeywordRepository(appManager : AppManager)
  extends
    RepositoryBase[Keyword, KeywordRow, Int](appManager) {
  ...
  override def setEntityId(
    entityId : Option[Int],
    entity   : Option[Tables.KeywordRow]) : Option[Tables.KeywordRow] = {
  ...
  override def getAddAction(
    entity : Tables.KeywordRow) =
    table returning table.map(_.keywordid) into
      ((entityProjection, entityId) => entityProjection.copy(keywordid = entityId)) += entity
  ...
  override def getQueryById(
    id : Int) = table.filter(c => c.keywordid === id)
