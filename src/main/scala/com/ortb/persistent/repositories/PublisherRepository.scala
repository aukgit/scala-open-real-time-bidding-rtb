...
class PublisherRepository(appManager : AppManager)
  extends
    RepositoryBase[Publisher, PublisherRow, Int](appManager) {
  ...
  override def setEntityId(
    entityId : Option[Int],
    entity   : Option[Tables.PublisherRow]) : Option[Tables.PublisherRow] = {
  ...
  override def getAddAction(
    entity : Tables.PublisherRow) =
    table returning table.map(_.publisherid) into
      ((entityProjection, entityId) => entityProjection.copy(publisherid = entityId)) += entity
  ...
  override def getQueryById(
    id : Int) = table.filter(c => c.publisherid === id)
