...
class ImpressionRepository(appManager : AppManager)
  extends
    RepositoryBase[Impression, ImpressionRow, Int](appManager) {
  ...
  override def setEntityId(
    entityId : Option[Int],
    entity   : Option[Tables.ImpressionRow]) : Option[Tables.ImpressionRow] = {
  ...
  override def getAddAction(
    entity : Tables.ImpressionRow) =
    table returning table.map(_.impressionid) into
      ((entityProjection, entityId) => entityProjection.copy(impressionid = entityId)) += entity
  ...
  override def getQueryById(
    id : Int) = table.filter(c => c.impressionid === id)
