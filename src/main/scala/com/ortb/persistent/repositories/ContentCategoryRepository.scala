...
class ContentCategoryRepository(appManager : AppManager)
  extends
    RepositoryBase[Contentcategory, ContentcategoryRow, String](appManager) {
  ...
  override def setEntityId(
    ...
    entity   : Option[Tables.ContentcategoryRow]) : Option[Tables.ContentcategoryRow] = {
  ...
  override def getAddAction(
    entity : Tables.ContentcategoryRow) =
    table returning table.map(_.contentcategoryid) into
      ((entityProjection, entityId) => entityProjection.copy(contentcategoryid = entityId)) += entity

  override def table = this.contentCategories

  override def getDeleteAction(
    entityId : String) : FixedSqlAction[Int, NoStream, Effect.Write] =
    getQueryById(entityId).delete

  override def getQueryById(
    id : String) = table.filter(c => c.contentcategoryid === id)
